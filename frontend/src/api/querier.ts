...
const querierSimulate = async (
  encodedMsgObjects: MsgExecuteContractEncodeObject[],
) => {
  ...
  const msgs = encodedMsgObjects.map((msgObject) => ({
    execute_contract: {
      ...msgObject.value,
      msg: msgObject.value.msg
        ? signedBytesEncode(msgObject.value.msg)
        : undefined,
    },
  }))
  ...
  >("/v1/chain/simulate", { messages: msgs }, { network: NETWORK_ID })
  ...
  const txRaw = await signer
    ...
    .catch((err) => {
      if (err instanceof Error && err.message === "Request rejected") {
        throw new AppError("Transaction rejected.", {
          level: "suppress",
          cause: err,
        })
      } else {
...
const isFound = (
  res: QuerierWaitForTxResponse,
): res is { found: QuerierExecuteTxResult } => {
  return "found" in res
}
  ...
  return poll(waitForTx, isFound)
    .catch((err) => {
      throw AppError.withCause(
        "Transaction not found in time. The chain might be congested.",
        err,
      )
    })
    .then((res) => res.found)
...
const querierBroadcastAndWait = async (
  address: string,
  signer: SigningCosmWasmClient,
  msgs: ExecuteMsg | ExecuteMsg[],
) => {
  ...
  return match(res)
    .with({ code: 0 }, () => res)
    .with({ code: 5 }, () => {
      throw AppError.withCause("You don't have enough gas funds.", res)
    })
    .with({ code: 11 }, () => {
      throw AppError.withCause("The transaction ran out of gas.", res)
    })
    .otherwise(() => {
      throw AppError.withCause(
        `The transaction failed (code ${res.code}).`,
        res,
      )
    })
}
...
export {
  ...
  querierExecuteTx,
  querierWaitForTx,
  querierBroadcastAndWait,
  querierAwaitCacheAnd,
}
