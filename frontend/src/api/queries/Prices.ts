...
const tokenPricesFromResponse = (response: TokenPricesResponse): TokenPrices => {
  const priceEntries: [Denom, BigNumber][] = []

  for (const [denom, tokenConfig] of tokenConfigs.entries()) {
    const responseToken = response.find(token => token.id === tokenConfig.pythId)?.price
    if (responseToken) {
      priceEntries.push([
        denom,
        BigNumber(responseToken.price).times(BigNumber(10).pow(responseToken.expo)),
      ])
    }
  }
